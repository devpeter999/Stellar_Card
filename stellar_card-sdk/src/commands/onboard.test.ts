...
describe('deriveDefaultWalletName', () => {
  ...
  it('produces a name prefixed with stellar_card-', () => {
    expect(_deriveDefaultWalletName(claimA, 'research-bot')).toMatch(/^stellar_card-/);
  });
  ...
  it('caps the label slug so a long label does not blow out the path', () => {
    ...
    // stellar_card- + slug (<=24) + - + 8-hex = ~42 chars max
  ...
  it('never contains characters unsafe for a filesystem vault path', () => {
    ...
    expect(name).toMatch(/^stellar_card-[a-z0-9-]+$/);
  });
});
