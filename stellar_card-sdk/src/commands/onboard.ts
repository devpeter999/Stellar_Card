// `stellar_card onboard --claim <code>` — one-shot agent setup.
//
...
// 2. Persist the api key + api_url in ~/.stellar_card/config.json (0600)
...
function usage(): void {
  process.stderr
    .write(`Usage: stellar_card onboard --claim <code> [--wallet-name <name>] [--vault-path <path>] [--passphrase-env <ENVNAME>] [--api-base <url>]

Exchanges a one-time claim code (from the stellar_card dashboard) for an
...
The raw api key is stored at ~/.stellar_card/config.json (chmod 0600) and
...
Options:
  ...
  --passphrase-env <ENVNAME> Name of the environment variable that holds the
                             ...
                             ~/.stellar_card/config.json, never the value itself.
  --api-base <url>           Override the default https://api.stellar_card.com/v1
 ...
 * running `stellar_card onboard` multiple times on the same machine gives
...
function deriveDefaultWalletName(claim: string, label: string | null): string {
  ...
  const slug =
    (label ?? 'agent')
      ...
      .replace(/^-+|-+$/g, '')
      .slice(0, 24) || 'agent';
  return `stellar_card-${slug}-${suffix}`;
}
...
export async function onboardCommand(argv: string[]): Promise<number> {
  ...
  let apiBase = args.apiBase || process.env.CARDS402_BASE_URL || 'https://api.stellar_card.com/v1';
  ...
  // If the machine already has a Stellar_Card config, warn before
  ...
  const existing = loadStellar_CardConfig();
  if (existing) {
    process.stderr.write(
      '⚠ An existing stellar_card config was found at ~/.stellar_card/config.json\n' +
  ...
  try {
    ...
    if (!res.ok) {
      ...
      process.stderr.write(
        'If the code is expired, ask your operator to mint a new one in the stellar_card dashboard.\n',
      );
      return 1;
    }
    ...
    if (typeof data.api_key !== 'string' || !data.api_key) {
      process.stderr.write(
        'error: claim succeeded but the response is missing api_key — aborting.\n' +
          'This is a backend bug. Report it to api@stellar_card.com with the response.\n',
      );
      return 1;
    }
    ...
    // to ~/.stellar_card/config.json and then crash every subsequent SDK
    // call via Node's ERR_INVALID_CHAR on the X-Api-Key header. The
    // load-time check in config.ts::loadStellar_CardConfig catches it on
    ...
    if (typeof data.api_key_id !== 'string' || !data.api_key_id) {
      process.stderr.write(
        'error: claim response missing api_key_id — aborting. Report to api@stellar_card.com.\n',
      );
      return 1;
    }
    ...
    // go to the attacker. Persisting the poisoned url to ~/.stellar_card/
  ...
  // default 'stellar_card-agent' name.
  ...
  // is persisted to config; the value never touches disk in stellar_card.
  ...
  const { path: configPath } = saveStellar_CardConfig({
  ...
  process.stdout.write('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
  process.stdout.write(' stellar_card agent ready\n');
  process.stdout.write('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
  ...
  if (isFunded) {
    ...
    if (usdcNum === 0) {
      ...
      process.stdout.write('    npx -y stellar_card@latest wallet trustline\n');
      process.stdout.write('\n');
    }
    process.stdout.write('  Try a test purchase:\n');
    process.stdout.write('    npx -y stellar_card@latest purchase --amount 0.01\n');
    if (usdcNum > 0) {
      process.stdout.write('\n');
    ...
    process.stdout.write('    npx -y stellar_card@latest purchase --amount <USD>\n');
  }
  process.stdout.write('\n');
  process.stdout.write('Your operator sees setup progress live in the stellar_card dashboard.\n');
  process.stdout.write('\n');
  return 0;
}
