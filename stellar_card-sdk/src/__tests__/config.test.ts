...
// that reads and writes ~/.stellar_card/config.json.
//
// Locks in the 2026-04-15 audit fixes:
//   F1  load refuses symbolic links (symlink attack defence)
//   F2  save re-chmods an existing ~/.stellar_card dir to 0700 instead
...
// ~/.stellar_card/config.json on the host.

let tmpDir: string;
let cfgPath: string;

beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'stellar_card-config-test-'));
...
describe('saveStellar_CardConfig → loadStellar_CardConfig round trip', () => {
  it('persists and reloads a minimal config', () => {
    const cfg: Stellar_CardConfig = {
      api_key: 'stellar_card_testkey',
      api_url: 'https://api.stellar_card.com/v1',
      created_at: new Date().toISOString(),
    };
    saveStellar_CardConfig(cfg, cfgPath);
    const loaded = loadStellar_CardConfig(cfgPath);
  ...
  it('returns null when the file does not exist', () => {
    const loaded = loadStellar_CardConfig(path.join(tmpDir, 'nope.json'));
...
describe('loadStellar_CardConfig — F1 symlink refusal', () => {
  it('throws on a symlink even if the target is a valid config', () => {
    ...
    expect(() => loadStellar_CardConfig(cfgPath)).toThrow(/symbolic link/i);
  });
...
describe('saveStellar_CardConfig — F2 dir mode repair', () => {
  it('tightens a pre-existing 0755 directory to 0700 on save', () => {
    ...
    // so the explicit chmod in saveStellar_CardConfig is what has to fix it.
...
describe('saveStellar_CardConfig — F3 temp file hygiene', () => {
...
describe('assertSafeBaseUrl — F4 embedded credentials', () => {
  it('accepts a bare https URL', () => {
    expect(assertSafeBaseUrl('https://api.stellar_card.com/v1')).toMatch(
      /^https:\/\/api\.stellar_card\.com\/v1/,
    );
  });
...
describe('assertSafeBaseUrl — protocol check', () => {
  it('rejects http without the override env', () => {
    ...
    expect(() => assertSafeBaseUrl('http://api.stellar_card.com/v1')).toThrow(/HTTPS/);
  });
...
describe('loadStellar_CardConfig — F5 size cap', () => {
  it('refuses a config file larger than 16 KB', () => {
    ...
    expect(() => loadStellar_CardConfig(cfgPath)).toThrow(/Refusing to load/i);
  });

  it('accepts a normal-sized config', () => {
    const cfg = {
      api_key: 'stellar_card_normal',
      api_url: 'https://api.stellar_card.com/v1',
      created_at: new Date().toISOString(),
    };
    writeCfg(cfg, 0o600);
    const loaded = loadStellar_CardConfig(cfgPath);
    expect(loaded?.api_key).toBe('stellar_card_normal');
  });
});
...
describe('resolveCredentials', () => {
  ...
  it('rejects a userinfo-spoofed CARDS402_BASE_URL at resolve time (F4)', () => {
    process.env.CARDS402_BASE_URL = 'https://api.stellar_card.com@evil.com/';
    process.env.CARDS402_API_KEY = 'stellar_card_whatever';
  ...
  it('uses explicit opts over env over config', () => {
    ...
    process.env.CARDS402_BASE_URL = 'https://env.stellar_card.com/v1';
