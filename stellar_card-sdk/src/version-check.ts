...
// locally-installed stellar_card is older than the current `latest` tag on
// the npm registry.
//
...
//     to `~/.stellar_card/version-check.json` and skip the registry entirely
...
const REGISTRY_URL = 'https://registry.npmjs.org/stellar_card/latest';
const CHECK_INTERVAL_MS = 24 * 60 * 60 * 1000; // once per day
const FETCH_TIMEOUT_MS = 2_000;
const STATE_FILE = path.join(os.homedir(), '.stellar_card', 'version-check.json');
...
// stellar_card. A hostile server (DNS hijack, MITM, compromised CDN) could
...
function printWarning(local: string, latest: string): void {
  ...
  const msg = [
    '',
    `⚠  stellar_card ${local} is out of date — ${latest} is available on npm.`,
    `   Upgrade: npx -y stellar_card@latest <command>  |  npm i -g stellar_card@latest`,
    `   Release notes: https://stellar_card.com/changelog`,
    '',
  ].join('\n');
  try {
