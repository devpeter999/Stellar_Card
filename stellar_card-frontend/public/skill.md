# Stellar_Card · Agent skill

You are an autonomous agent that has been given access to Stellar_Card — a service
...
## What Stellar_Card does

Stellar_Card lets you buy a Visa reward card, funded in USDC or XLM over the
...
(documented at <https://stellar_card.com/pricing>).
...
```bash
npx -y stellar_card@latest onboard --claim c402_<code>
```
...
2. Writes the key to `~/.stellar_card/config.json` (chmod 0600) so the SDK can
...
4. Reports the wallet address to the stellar_card backend so your operator sees
...
```bash
npx -y stellar_card@latest wallet balance
```
...
```bash
npx -y stellar_card@latest wallet trustline
```
...
If you only intend to pay stellar_card in XLM you can skip this step — XLM
...
XLM). Once the balance updates, `stellar_card purchase` works.

After onboarding, any subsequent Stellar_Card command uses the cached key
...
Stellar_Card charges no markup to agents — you pay face value and get a
...
stellar_card accepts any order from **$0.01 to $10,000** per card. Below the
...
Stellar_Card returns errors as JSON bodies with a stable `error` field. The
...
Stellar_Card is designed to run without a human, but you should escalate if:
...
- A purchase would push the card balance above $10,000 (not possible, but
  worth checking).
- You repeatedly get `service_temporarily_unavailable` — this means Stellar_Card
...
- Pricing and limits: <https://stellar_card.com/pricing>
- Full HTTP API reference: <https://stellar_card.com/docs>
- Quickstart guide: <https://stellar_card.com/docs/quickstart>
- Security posture: <https://stellar_card.com/security>
- Cardholder agreement (issuer terms): <https://stellar_card.com/legal/cardholder-agreement>

## A note on trust

Stellar_Card does not store API keys in plaintext — they are hashed (bcrypt)
...
running the SDK. Stellar_Card never sees or touches the secret.

If you ever need to revoke an agent's access, open
<https://stellar_card.com/dashboard>, go to the **Agents** tab, and click
