...
export const metadata: Metadata = {
  title: 'Compare',
  description:
    'How Stellar_Card compares to traditional corporate cards, shared team cards, and other agent-payment rails. Honest trade-offs, not a sales table.',
  alternates: { canonical: 'https://stellar_card.com/compare' },
  openGraph: ogForPage({
    title: 'Compare — Stellar_Card',
    description:
      'Stellar_Card versus corporate cards, shared team cards, and other agent-payment rails.',
    path: '/compare',
  }),
...
// Feature rows for the head-to-head matrix. `stellar_card` is the canonical
...
// every "worse" value for Stellar_Card is explicitly noted, not hidden.
...
const ROWS: Row[] = [
  ...
  {
    label: 'Card issuance',
    stellar_card: { value: 'Per purchase', note: '~60s on mainnet', win: true },
  ...
  {
    label: 'Spend control',
    stellar_card: { value: 'Per-key spend cap', note: 'Policy-gated approvals optional' },
  ...
  },
  {
    ...
    stellar_card: { value: '$10,000', note: "Pathward's per-card balance ceiling" },
  ...
  },
  {
    ...
    stellar_card: { value: 'Yes', note: '$2 + 2% foreign txn fee' },
...
const SCENARIOS = [
  {
    ...
    verdict:
      'Either a shared team card or Stellar_Card works. Shared is faster to set up; Stellar_Card is safer if the agent is LLM-driven.',
  },
  {
    title: 'You have ten agents and want per-agent blast radius',
    verdict:
      'Stellar_Card. Shared team cards collapse into one compromised credential; corporate-card provisioning takes days per agent.',
  },
  {
    title: 'You need a single recurring subscription paid from one card',
    verdict: 'Not Stellar_Card — Pathward reward cards block recurring charges. Use a corporate card.',
  },
  {
    title: 'You want predictable cost per transaction without platform markup',
    verdict: 'Stellar_Card. Face value + issuer fees only, no per-order platform cut.',
  },
  {
    title: 'Your agents run on stablecoin-native infrastructure',
    verdict:
      'Stellar_Card. Skip fiat on/off ramps entirely — pay the receiver contract directly in USDC or XLM.',
  },
  {
    title: 'You need a physical card or ATM access',
    verdict:
      'Not Stellar_Card. Reward cards are virtual-only with no cash access. Use a corporate card.',
  },
];
...
// honest — we don't paint every Stellar_Card cell green.
...
export default function ComparePage() {
  return (
    <>
      <PageHero
        eyebrow="Compare"
        title="Stellar_Card vs the"
        accent="alternatives"
        intro="Stellar_Card isn't the right answer for every agent-payment use case. This page is the honest matrix we'd show in a sales call — every row where a corporate card or a shared team card wins is explicitly marked. Use it to pick the right tool for the job you actually have."
      />
      ...
      <PageSection>
            ...
            <tbody>
              {ROWS.map((row) => (
                <tr key={row.label} style={{ borderBottom: '1px solid var(--border-hairline)' }}>
                  ...
                  <MatrixCell cell={row.stellar_card} />
        ...
        >
          ...
          think Stellar_Card genuinely wins. The others are either tied or lost — we&apos;re not
      ...
      <section style={{ padding: '3rem 1.35rem 6rem' }}>
        ...
        >
          ...
          <div style={{ position: 'relative' }}>
            ...
            >
              Email us — we&apos;ll tell you if Stellar_Card is wrong for your use case.
            </h2>
            <p
              className="type-body"
            ...
            >
              A short email to{' '}
              <a
                href="mailto:hello@stellar_card.com"
                style={{
                  color: 'var(--fg)',
                  textDecoration: 'none',
