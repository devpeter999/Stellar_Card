...
export const metadata: Metadata = {
  title: 'Quickstart',
  description:
    'Issue your first Stellar_Card card in five minutes. Install the SDK, claim a key, pay a Soroban contract, stream the card.',
  alternates: { canonical: 'https://stellar_card.com/docs/quickstart' },
};
  ...
  itemListElement: [
    {
      ...
      position: 1,
      name: 'Docs',
      item: 'https://stellar_card.com/docs',
    },
    {
      '@type': 'ListItem',
      position: 2,
      name: 'Quickstart',
      item: 'https://stellar_card.com/docs/quickstart',
    },
  ],
};
...
const howToJsonLd = {
  '@context': 'https://schema.org',
  '@type': 'HowTo',
  name: 'Issue your first Stellar_Card card in five minutes',
  description:
    'Install the Stellar_Card SDK, claim an API key, fund a Stellar wallet, purchase a card, and wire it into an agent runtime.',
  totalTime: 'PT5M',
  supply: [
    { '@type': 'HowToSupply', name: 'A Stellar_Card claim code from an operator' },
  ...
  step: [
    {
      ...
      text: 'Run npm install stellar_card — the single package ships the TypeScript SDK, the CLI, and the MCP server.',
      url: 'https://stellar_card.com/docs/quickstart#install-the-sdk',
    },
    {
      '@type': 'HowToStep',
      position: 2,
      name: 'Claim your first API key',
      text: 'Exchange a single-use claim code for an API key via npx -y stellar_card@latest onboard --claim c402_<code>. Always pin @latest on npx so you re-resolve against the registry and never run a stale cached version of the CLI.',
      url: 'https://stellar_card.com/docs/quickstart#claim-your-first-api-key',
    },
    {
      '@type': 'HowToStep',
      position: 3,
      name: 'Fund a wallet',
      text: 'Create a Stellar wallet via createOWSWallet() and fund it with at least 2 XLM (1 XLM minimum balance + 0.5 XLM USDC trustline reserve + headroom). Open the USDC trustline via `stellar_card wallet trustline` before the operator sends USDC — USDC is an issued asset on Stellar and payments to a wallet without a trustline bounce back to the sender.',
      url: 'https://stellar_card.com/docs/quickstart#fund-a-wallet',
    },
    {
      '@type': 'HowToStep',
      position: 4,
      ...
      url: 'https://stellar_card.com/docs/quickstart#purchase-your-first-card',
    },
    {
      '@type': 'HowToStep',
      position: 5,
      name: 'Wire it into your agent',
      text: 'Add the Stellar_Card MCP server to your claude_desktop_config.json or other MCP host so the purchase_vcc tool is available to the LLM.',
      url: 'https://stellar_card.com/docs/quickstart#wire-it-into-your-agent',
    },
  ],
};
...
const STEPS = [
  {
    ...
    body: (
      <>
        <p>
          Stellar_Card ships as a single npm package with an included CLI. One install gets you the
          TypeScript SDK, the CLI, and the MCP server.
        </p>
        <CodeBlock label="Shell">{`npm install stellar_card@latest
# or: pnpm add stellar_card@latest / bun add stellar_card@latest`}</CodeBlock>
        <p>
          The package exports <Code>purchaseCardOWS</Code>, <Code>createOrder</Code>,{' '}
          <Code>waitForCard</Code>, and a <Code>Stellar_Card</Code> client class for lower-level usage.
        </p>
        <p>
        ...
        <CodeBlock label="Shell">{`npx -y stellar_card@latest --help`}</CodeBlock>
        <p>
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <CodeBlock label="Shell">{`npx -y stellar_card@latest onboard --claim c402_abc123...`}</CodeBlock>
        <p>
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <CodeBlock label="TypeScript">{`import { createOWSWallet, getOWSBalance } from 'stellar_card';
...
const { xlm, usdc } = await getOWSBalance('my-agent');`}</CodeBlock>
        ...
        <CodeBlock label="CLI">{`npx -y stellar_card@latest wallet trustline`}</CodeBlock>
        <p>Or from TypeScript:</p>
        <CodeBlock label="TypeScript">{`import { addUsdcTrustlineOWS } from 'stellar_card';
...
console.log('trustline opened:', txHash);`}</CodeBlock>
        ...
        <p>
          Stellar_Card never sees or touches the secret key — it lives in an encrypted OWS vault on the
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <CodeBlock label="TypeScript">{`import { purchaseCardOWS } from 'stellar_card';
...
// }`}</CodeBlock>
  ...
  },
  {
    ...
    body: (
      <>
        <p>
          Stellar_Card ships an MCP server so Claude Desktop and any other MCP-aware runtime can call it
...
export default function QuickstartPage() {
  return (
    ...
    >
      ...
      >
        This walk-through covers a full Stellar_Card integration from scratch: install the SDK, claim a
