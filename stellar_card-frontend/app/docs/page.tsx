...
export const metadata: Metadata = {
  title: 'API reference',
  description:
    'Full API reference for Stellar_Card: create orders, stream status, verify webhooks, and handle errors.',
  alternates: { canonical: 'https://stellar_card.com/docs' },
};
...
const techArticleJsonLd = {
  ...
  '@type': 'TechArticle',
  '@id': 'https://stellar_card.com/docs#article',
  headline: 'Stellar_Card HTTP API reference',
  description:
    'Full HTTP API for Stellar_Card: create orders, stream order state over SSE, poll as a fallback, verify webhook signatures, and handle every error code.',
  ...
  author: {
    '@type': 'Organization',
    name: 'Stellar_Card',
    url: 'https://stellar_card.com',
  },
  publisher: { '@id': 'https://stellar_card.com#organization' },
  inLanguage: 'en-GB',
  mainEntityOfPage: 'https://stellar_card.com/docs',
};
      ...
      <aside className="docs-sidebar">
        <div className="docs-sidebar-inner">
          ...
          >
            ...
            <a
              href="mailto:api@stellar_card.com"
              style={{
                fontSize: '0.8rem',
                color: 'var(--fg-muted)',
                textDecoration: 'none',
      ...
      <div className="docs-main">
        ...
        <header style={{ padding: '4rem 0 3.5rem' }}>
          ...
          >
            ...
            <span style={{ color: 'var(--green)' }}>https://api.stellar_card.com</span>
          </div>
        </header>
        ...
        <Section id="authentication" eyebrow="01 · Authentication" title="One header, one key.">
          <Para>
            ...
            <Code>stellar_card_</Code> and scoped to a spend limit in USDC.
          </Para>
          <Para>
            ...
            agent to run <Code>stellar_card onboard --claim &lt;code&gt;</Code>. The agent exchanges the
            claim for a real API key — the raw token is never shown in the dashboard, only stored as
            a salted hash.
          </Para>
          <CodeBlock label="Header">{`X-Api-Key: stellar_card_a1b2c3d4e5f6...`}</CodeBlock>
          <Para>
        ...
        >
          ...
          <CodeBlock label="Request">
            {`POST /v1/orders
X-Api-Key: stellar_card_your_key_here
...

{
  ...
  "payment": {
    ...
    "contract_id": "CAAAA...stellar_card_receiver_contract...",
    "order_id": "a3f7c2d1-4e8b-4f0a-9c2d-1b3e5a7f9c0e",
    "usdc": {
      "amount": "25.0000000",
  ...
  "expires_at": "2026-04-08T14:00:00.000Z"
          ...
          <Para>
            ...
            <Code>stellar_card onboard --claim &lt;code&gt;</Code>, a single command handles
            create-order → sign Soroban tx → stream → return card:
          </Para>
          <CodeBlock label="Shell">
            {`npx -y stellar_card@latest purchase --amount 25
...
def wait_for_card(api_url: str, order_id: str, key: str):
    ...
    raise RuntimeError("stream ended before terminal phase")`}
          ...
          <CodeBlock label="Minimal client — curl">
            {`curl -N \\
  -H "X-Api-Key: stellar_card_..." \\
  -H "Accept: text/event-stream" \\
  https://api.stellar_card.com/v1/orders/<order_id>/stream`}
          </CodeBlock>

          <Para>
            The Stellar_Card SDK&apos;s <Code>waitForCard()</Code> already uses this path with polling
  ...
  "updated_at": "2026-04-08T12:05:27.000Z"
          ...
          <Para style={{ color: 'var(--fg-dim)', fontSize: '0.85rem' }}>
            ...
            event). Use it as the join key between Stellar_Card orders and your own internal records.
          </Para>
  ...
  },
  {
    ...
    "updated_at": "2026-04-08T11:59:12.000Z"
        ...
        <Section id="webhooks" eyebrow="06 · Webhooks" title="Signed, retried, optional.">
          <Para>
            If you provide a <Code>webhook_url</Code> when creating an order, Stellar_Card will POST to
          ...
          >
            <li style={{ marginBottom: '0.35rem' }}>
              <Code>X-Stellar_Card-Signature: sha256=&lt;hmac&gt;</Code> — HMAC-SHA256 over{' '}
              <Code>&lt;timestamp&gt;.&lt;body&gt;</Code>
            </li>
            <li>
              <Code>X-Stellar_Card-Timestamp: &lt;unix-ms&gt;</Code> — send time in milliseconds
            </li>
          </ul>
          <Para>
...
def verify_webhook(raw_body: bytes, signature: str, timestamp: str, secret: str) -> bool:
  ...
  "message": "amount_usdc must be a positive number"
          ...
          <Para>
            The Stellar_Card SDK converts the <Code>error</Code> string from each response into a typed
            subclass of <Code>Stellar_CardError</Code>, so agents can handle each case without
            string-parsing:
          </Para>
...
try {
  ...
  if (err instanceof SpendLimitError) {
    ...
    // err.orderId — resume with stellar_card purchase --resume <id>
  } else if (err instanceof AuthError) {
    // key is missing or revoked
  } else {
    // Stellar_CardError base, or a non-stellar_card error — rethrow
    throw err;
  }
}`}
          </CodeBlock>
          ...
          <Para style={{ color: 'var(--fg-dim)', fontSize: '0.85rem' }}>
            All typed errors extend <Code>Stellar_CardError</Code>, which carries <Code>code</Code>,{' '}
