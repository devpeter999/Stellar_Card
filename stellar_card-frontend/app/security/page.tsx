...
export const metadata: Metadata = {
  title: 'Security',
  description:
    'How Stellar_Card secures API keys, payments, and infrastructure. Non-custodial by design, hashed keys, signed webhooks, and a responsible-disclosure policy.',
  alternates: { canonical: 'https://stellar_card.com/security' },
  openGraph: ogForPage({
    title: 'Security — Stellar_Card',
...
const PILLARS = [
  {
    ...
    body: 'Agents pay the Soroban receiver contract directly. Stellar_Card never holds funds — we observe on-chain events and broker fulfilment. If Stellar_Card disappeared tomorrow, nothing is trapped in our custody, because nothing is in our custody.',
  },
  {
    eyebrow: 'Keys',
  ...
  },
  {
    ...
    body: 'The stellar_card dashboard mints single-use claim codes so operators never paste API keys into LLM context. The agent redeems the code for a key on its own machine, over TLS, and the code is invalidated after use. No credential lives in the transcript.',
  },
  {
    eyebrow: 'Webhooks',
    title: 'HMAC signed, replay protected.',
    body: 'Outgoing webhooks carry X-Stellar_Card-Signature (HMAC-SHA256 over timestamp + body) and X-Stellar_Card-Timestamp. The documented client reference rejects anything older than five minutes. Webhook secrets rotate automatically on key revocation.',
  },
  {
  ...
  return (
    <>
      <PageHero
        ...
        accent="trust"
        intro="Stellar_Card is a small team running financial infrastructure. Everything below is a design choice, not a marketing bullet — we picked these specifically so a single compromise of any one component never exposes customer funds or credentials."
      />
      ...
      <PageSection eyebrow="Disclosure" title="Found something? Tell us. We'll pay.">
        <div style={{ maxWidth: 720 }}>
          <p className="type-body" style={{ fontSize: '0.98rem', marginBottom: '1.5rem' }}>
            Stellar_Card operates a responsible-disclosure programme. If you find a vulnerability, email{' '}
            <a
              href="mailto:security@stellar_card.com"
              style={{
                color: 'var(--fg)',
