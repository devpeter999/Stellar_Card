...
export const metadata: Metadata = {
  title: 'Status',
  description:
    'Live operational status for the Stellar_Card HTTP API, fulfilment pipeline, and Stellar watcher. Fetched from api.stellar_card.com/status on every page load.',
  alternates: { canonical: 'https://stellar_card.com/status' },
  openGraph: ogForPage({
    title: 'Status — Stellar_Card',
    description: 'Live operational status for the Stellar_Card API and pipeline.',
    path: '/status',
  }),
...
// fetching api.stellar_card.com/status, so backend schema drift can't
...
async function fetchBackendStatus(): Promise<BackendStatus | null> {
  const url = process.env.NEXT_PUBLIC_API_BASE_URL
    ...
    : 'https://api.stellar_card.com/status';
  try {
...
export default async function StatusPage() {
  ...
  const message = !s
    ? "Couldn't reach api.stellar_card.com/status. The status endpoint may be down, or your network blocks it. The marketing surface at stellar_card.com is unaffected."
    : overall === 'operational'
  ...
  return (
    <>
      <PageHero
        ...
        intro={
          <>
            Real-time health of every Stellar_Card component. This page fetches{' '}
            <code
              style={{
      ...
      <PageSection>
        {rows ? (
          ...
          >
            The status endpoint at <code>api.stellar_card.com/status</code> is not responding. Try again
