// End-to-end V2 test: create order via stellar_card API → pay Soroban contract → poll for card
...
async function main() {
  console.log(`\n=== stellar_card V2 E2E test — $${AMOUNT} USDC ===\n`);
  ...
  // Format: stellar_card_<48 hex chars> — matches the key_prefix fast-path in auth.js
  const testToken = `stellar_card_${crypto.randomBytes(24).toString('hex')}`;
  ...
  try {
    ...
    // ── 4. Poll stellar_card until delivered ──────────────────────────────────────
