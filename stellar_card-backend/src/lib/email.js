...
//
// Visual language (aligned with the Stellar_Card brand refresh):
//   - Darker canvas (#050505) and softer borders so the emails read
//     engineering-grade rather than marketing-flashy
//   - Stellar_Card wordmark pulled from https://stellar_card.com/logo.svg with a
...
// "Stellar_Card" rather than the raw no-reply address. If the operator already
// set a display name in SMTP_FROM (e.g. "foo <bar@baz.com>"), keep it.
function from() {
  const raw = process.env.SMTP_FROM || '';
  if (raw.includes('<')) return raw;
  return `"Stellar_Card" <${raw}>`;
}
...
const LOGO_URL = 'https://stellar_card.com/logo-light.svg';
...
const DASHBOARD_URL = 'https://stellar_card.com/dashboard';

// ── Shared chrome ─────────────────────────────────────────────────────────────

// The header renders the Stellar_Card wordmark as an SVG <img> with a
...
// readable "Stellar_Card" in serif.
function header() {
  return `
    <tr>
      <td style="padding:32px 36px 0 36px;">
        <a href="https://stellar_card.com" style="text-decoration:none;color:${COLOR.text};" aria-label="Stellar_Card">
          <img
            src="${LOGO_URL}"
            width="120"
            height="28"
      ...
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;width:100%;background-color:${COLOR.card};border:1px solid ${COLOR.border};border-radius:14px;">
        ...
        <tr>
          <td style="padding:20px 36px;border-top:1px solid ${COLOR.border};font-family:${FONT_SANS};font-size:12px;color:${COLOR.faint};">
            <a href="https://stellar_card.com" style="color:${COLOR.faint};text-decoration:none;">stellar_card.com</a>
            &nbsp;·&nbsp;
...
async function sendLoginCode(email, code) {
  ...
  const body = `
    ${eyebrow('Sign in')}
    ${headline('Your Stellar_Card login code')}
  ...
  const subject = 'Your Stellar_Card login code';
  await sendMailStrict(getTransporter(), {
    from: from(),
    to: email,
    subject,
    text: [
      `Your Stellar_Card login code:`,
      ``,
      `    ${code}`,
      ``,
...
async function sendApprovalEmail(
  ...
  const safeSubject = `Stellar_Card — approval required for $${sanitizeHeader(amountUsdc)} USDC`;
  await sendMailStrict(getTransporter(), {
    from: from(),
    to: ownerEmail,
    subject: safeSubject,
...
async function sendSpendAlertEmail(ownerEmail, { keyLabel, pct, spentUsdc, limitUsdc, limitType }) {
  ...
  const safeSubject = `Stellar_Card — ${safeLabel} at ${safePct}% of ${safeLimitType} limit`;
  await sendMailStrict(getTransporter(), {
    from: from(),
    to: ownerEmail,
    subject: safeSubject,
...
async function sendAlertEmail({ to, subject, body }) {
  ...
  // lib/alerts.js::deliverFiring as `stellar_card alert: ${rule.name}`,
