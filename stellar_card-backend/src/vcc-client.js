...
// canonical key for at-rest encryption across stellar_card (see
...
// Audit C-7: simple circuit breaker on the stellar_card → vcc path. If vcc is
...
async function getVccToken() {
  ...
  const label = process.env.VCC_INSTANCE_LABEL || `stellar_card-${process.env.NODE_ENV || 'prod'}`;
  let res;
  try {
...
// where paymentUrl is a web+stellar:pay URI that stellar_card must pay directly.
//
...
async function getInvoice(orderId, amountUsdc, requestId = null, callbackNonce = null) {
  ...
  // Audit C-3: per-job nonce. Generated by stellar_card at invoice time, stored
  ...
  // payment_url is the Stellar URI that stellar_card pays next — a malformed
...
// Tells VCC that stellar_card has paid CTX. VCC transitions the job to queued and
