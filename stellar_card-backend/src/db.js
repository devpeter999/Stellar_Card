...
const DB_PATH = process.env.DB_PATH || path.join(__dirname, '..', 'stellar_card.db');
...
//
// The agent runs `npx stellar_card onboard --claim <code>`. The CLI hits
...
// config file (~/.stellar_card/config.json) and the SDK loads from there.
//
...
applyMigration(15, () => {
  db.exec(`
    CREATE TABLE IF NOT EXISTS agent_claims (
      ...
      -- Contains the raw 'stellar_card_...' api key so the claim endpoint can
