// @ts-check
// Refund and webhook delivery for the stellar_card backend.
...
async function fireWebhook(url, payload, webhookSecret, _log, context = {}) {
  ...
  if (webhookSecret) {
    ...
    headers['X-Stellar_Card-Signature'] = `sha256=${sig}`;
    headers['X-Stellar_Card-Timestamp'] = ts;
    signatureHeader = headers['X-Stellar_Card-Signature'];
  }
...
// authoritative: if ctx_stellar_txid is populated, stellar_card has paid
...
// stage-2 chromium launch made VCC mark three orders failed, stellar_card
