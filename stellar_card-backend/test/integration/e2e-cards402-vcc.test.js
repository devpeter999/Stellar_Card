// End-to-end integration test spanning stellar_card and vcc.
//
...
// any assertion. This test stands up a fake vcc HTTP server, points stellar_card
// at it via VCC_API_BASE, and walks an order all the way from creation to
// delivery.
//
// Flow per test:
//   1. Agent creates an order via POST /v1/orders  (stellar_card HTTP)
...
before(async () => {
  ...
  // 2. Point stellar_card's vcc-client at it BEFORE any app modules load,
...
describe('e2e stellar_card ↔ vcc: happy path', () => {
  it('agent creates order → pays contract → vcc delivers card → agent sees phase: ready', async () => {
    ...
    // 6. vcc scrapes and fires the HMAC-signed callback to stellar_card.
...
describe('e2e stellar_card ↔ vcc: vcc reports failure', () => {
  it('vcc callback with status=failed AFTER ctx_stellar_txid is set → quarantine (no refund)', async () => {
    ...
    // stellar_card had forwarded payment to CTX. The gift card existed on
  ...
  it('vcc callback with status=failed AND no ctx_stellar_txid → standard auto-refund', async () => {
    // Control case: VCC reports failure BEFORE stellar_card forwarded payment
...
describe('e2e stellar_card ↔ vcc: duplicate payment event', () => {
...
describe('e2e stellar_card ↔ vcc: F1 ambiguous CTX payment parking', () => {
...
describe('e2e stellar_card ↔ vcc: per-order callback secret (audit F2)', () => {
...
describe('e2e stellar_card ↔ vcc: payment amount validation (audit F0/F7)', () => {
  ...
  it('routes payments for unknown order_ids to unmatched_payments', async () => {
    ...
    // and tries to get stellar_card to treat it as fulfillable.
...
describe('e2e stellar_card ↔ vcc: terminal callback ignored', () => {
