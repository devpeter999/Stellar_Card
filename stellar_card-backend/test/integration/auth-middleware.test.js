...
describe('auth middleware — /v1/* api key verification', () => {
  ...
  it('rejects a header that is not a valid stellar_card key format', async () => {
    const res = await request.get('/v1/usage').set('X-Api-Key', 'not-a-real-key');
    assert.equal(res.status, 401);
    assert.equal(res.body.error, 'invalid_api_key');
  });

  it("rejects a key missing the 'stellar_card_' prefix", async () => {
  ...
  it('rejects a key shorter than the minimum length', async () => {
    ...
    const res = await request.get('/v1/usage').set('X-Api-Key', 'stellar_card_short');
    assert.equal(res.status, 401);
    assert.equal(res.body.error, 'invalid_api_key');
  });

  it('rejects a key longer than the maximum length', async () => {
    const res = await request.get('/v1/usage').set('X-Api-Key', 'stellar_card_' + 'a'.repeat(200));
    assert.equal(res.status, 401);
    assert.equal(res.body.error, 'invalid_api_key');
  });
  ...
  it('handles an X-Api-Key header that is an array (duplicated header)', async () => {
    ...
    const res = await request
      .get('/v1/usage')
      .set('X-Api-Key', 'stellar_card_' + 'a'.repeat(48))
      .set('X-Api-Key', 'stellar_card_' + 'b'.repeat(48));
    assert.equal(res.status, 401);
    assert.equal(res.body.error, 'invalid_api_key');
  });
