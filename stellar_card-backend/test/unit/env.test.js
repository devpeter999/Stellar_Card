...
// `https://api.stellar_card.test`, which is legitimate under the F5-env
...
function baseEnv() {
  return {
    ...
    CARDS402_BASE_URL: 'https://api.stellar_card.test',
...
describe('F2-env: INTERNAL_EMAILS per-entry validation', () => {
  it('accepts a single valid email', () => {
    const r = _EnvSchema.safeParse({
      ...baseEnv(),
      INTERNAL_EMAILS: 'ops@stellar_card.com',
    });
    assert.equal(r.success, true);
  });
  ...
  it('accepts a comma-separated list of valid emails', () => {
    const r = _EnvSchema.safeParse({
      ...baseEnv(),
      INTERNAL_EMAILS: 'ops@stellar_card.com,dev@stellar_card.com,another@example.org',
    });
    assert.equal(r.success, true);
  });
  ...
  it('trims whitespace and lowercases', () => {
    const r = _EnvSchema.safeParse({
      ...baseEnv(),
      INTERNAL_EMAILS: '  Ops@Stellar_Card.COM ,  Dev@stellar_card.com ',
    });
    assert.equal(r.success, true);
    assert.deepEqual(r.data.INTERNAL_EMAILS, ['ops@stellar_card.com', 'dev@stellar_card.com']);
  });
  ...
  it('rejects a typo missing the @', () => {
    const r = _EnvSchema.safeParse({
      ...baseEnv(),
      INTERNAL_EMAILS: 'opsstellar_card.com',
    });
    assert.equal(r.success, false);
  ...
  it('rejects a typo missing the dot', () => {
    const r = _EnvSchema.safeParse({
      ...baseEnv(),
      INTERNAL_EMAILS: 'ops@stellar_cardcom',
    });
    assert.equal(r.success, false);
  });
  ...
  it('rejects a list where ONE entry is bad (fail-loud for typos)', () => {
    const r = _EnvSchema.safeParse({
      ...baseEnv(),
      INTERNAL_EMAILS: 'good@stellar_card.com,broken-entry,another@stellar_card.com',
    });
    assert.equal(r.success, false);
  });
  ...
  it('ignores empty entries from trailing commas', () => {
    ...
    assert.equal(r.success, true);
    assert.deepEqual(r.data.INTERNAL_EMAILS, ['ops@stellar_card.com']);
  });
});
...
describe('F3-env: CORS_ORIGINS per-entry validation', () => {
  ...
  it('accepts a comma-separated list of https URLs', () => {
    const r = _EnvSchema.safeParse({
      ...baseEnv(),
      CORS_ORIGINS: 'https://stellar_card.com,https://dash.stellar_card.com',
    });
    assert.equal(r.success, true);
  });
  ...
  it('rejects a ftp:// origin', () => {
    const r = _EnvSchema.safeParse({
      ...baseEnv(),
      CORS_ORIGINS: 'ftp://stellar_card.com',
    });
    assert.equal(r.success, false);
  });
...
describe('F4-env: http(s)-only URL fields', () => {
  it('accepts https:// CARDS402_BASE_URL on a reserved test TLD', () => {
    ...
    // production URL like stellar_card.com would correctly trip that
  ...
  it('rejects ftp:// CARDS402_BASE_URL', () => {
    const r = _EnvSchema.safeParse({
      ...baseEnv(),
      CARDS402_BASE_URL: 'ftp://stellar_card.com',
    });
    assert.equal(r.success, false);
...
describe('F5-env: reserved TLDs are treated as local', () => {
  it('accepts https://api.stellar_card.test under NODE_ENV=test', () => {
    const r = _EnvSchema.safeParse({
      ...baseEnv(),
      CARDS402_BASE_URL: 'https://api.stellar_card.test',
    });
  ...
  it('accepts https://api.stellar_card.localhost under NODE_ENV=test', () => {
    const r = _EnvSchema.safeParse({
      ...baseEnv(),
      CARDS402_BASE_URL: 'https://api.stellar_card.localhost',
    });
    assert.equal(r.success, true);
  });
  ...
  it('rejects https://api.stellar_card.com under NODE_ENV=test (real prod URL)', () => {
    // The guard still correctly fires on a real production URL — this
    // is the whole point of the check and must not regress.
    const r = _EnvSchema.safeParse({
      ...baseEnv(),
      CARDS402_BASE_URL: 'https://api.stellar_card.com',
    });
    assert.equal(r.success, false);
    assert.ok(issueFor(r, 'NODE_ENV'));
  });

  it('accepts https://api.stellar_card.com under NODE_ENV=production', () => {
