...
describe('requireCardReveal — auth gate', () => {
  ...
  it('401 when req.user has no email (F2 defence)', () => {
    ...
    process.env.CARDS402_CARD_REVEAL_EMAILS = 'ops@stellar_card.com';
    const mw = freshMiddleware();
...
describe('requireCardReveal — allowlist', () => {
  ...
  it('403 card_reveal_disabled when env var is empty string', () => {
    ...
    const mw = freshMiddleware();
    const { statusCode, body } = runMiddleware(mw, { id: 'u1', email: 'ops@stellar_card.com' });
    assert.equal(statusCode, 403);
    assert.equal(body.error, 'card_reveal_disabled');
  });
  ...
  it('calls next() when user email is in the allowlist', () => {
    process.env.CARDS402_CARD_REVEAL_EMAILS = 'ops@stellar_card.com,other@stellar_card.com';
    const mw = freshMiddleware();
  ...
  it('trims whitespace around env entries', () => {
    process.env.CARDS402_CARD_REVEAL_EMAILS = '  ops@stellar_card.com  ,  other@stellar_card.com  ';
    const mw = freshMiddleware();
    const { nextCalled } = runMiddleware(mw, { id: 'u1', email: 'other@stellar_card.com' });
    assert.equal(nextCalled, true);
  });

  it('re-reads env on every call (supports live rotation)', () => {
    const mw = freshMiddleware();
    process.env.CARDS402_CARD_REVEAL_EMAILS = 'first@stellar_card.com';
    assert.equal(runMiddleware(mw, { id: 'u1', email: 'first@stellar_card.com' }).nextCalled, true);
    // Rotate without re-requiring the module.
    process.env.CARDS402_CARD_REVEAL_EMAILS = 'second@stellar_card.com';
    assert.equal(
      runMiddleware(mw, { id: 'u1', email: 'first@stellar_card.com' }).nextCalled,
      false,
      'first@ should be rejected after rotation',
    );
    assert.equal(
      runMiddleware(mw, { id: 'u1', email: 'second@stellar_card.com' }).nextCalled,
      true,
...
describe('F1-card-reveal: bizEvent observability', () => {
  ...
  it('emits card_reveal.denied with reason=env_not_configured when allowlist is empty', () => {
    ...
    const mw = freshMiddleware();
    runWithReq(mw, {
      user: { id: 'u1', email: 'ops@stellar_card.com' },
      ip: '10.0.0.3',
      headers: {},
    });
    const denied = events.find((e) => e.name === 'card_reveal.denied');
    ...
    assert.equal(denied.fields.actor_email, 'ops@stellar_card.com');
  ...
  it('emits card_reveal.allowed on successful authorization', () => {
    ...
    assert.equal(allowed.fields.actor_email, 'ops@stellar_card.com');
...
describe('F3-card-reveal: bizEvent failure does not alter auth verdict', () => {
  ...
  it('still calls next() on allowed branch even though bizEvent throws', () => {
    process.env.CARDS402_CARD_REVEAL_EMAILS = 'ops@stellar_card.com';
    const mw = freshMiddleware();
    const req = { user: { id: 'u1', email: 'ops@stellar_card.com' }, ip: '10.0.0.1', headers: {} };
    let statusCode = null;
    const res = {
      status(c) {
        statusCode = c;
