...
function seedUser(email = 'user@stellar_card.com', role = 'owner') {
  const id = uuidv4();
...
describe('requireAuth — baseline', () => {
  ...
  it('accepts a valid token and attaches req.user', () => {
    const userId = seedUser('alice@stellar_card.com', 'owner');
    ...
    assert.equal(req.user.email, 'alice@stellar_card.com');
