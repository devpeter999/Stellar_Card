// Tests for stellar_card fulfillment.js — refunds, webhooks, and freeze state.
// Card delivery itself happens in the VCC service; this file covers what
// stellar_card still owns: scheduleRefund, fireWebhook, enqueueWebhook, isFrozen.
...
describe('fireWebhook', () => {
  ...
  it('adds HMAC signature headers when webhookSecret is provided', async () => {
    ...
    assert.ok(
      opts.headers['X-Stellar_Card-Signature']?.startsWith('sha256='),
      'signature header present',
    );
    assert.ok(opts.headers['X-Stellar_Card-Timestamp'], 'timestamp header present');

    // Verify the HMAC is correct
    const ts = opts.headers['X-Stellar_Card-Timestamp'];
    const body = opts.body;
    const expected = crypto.createHmac('sha256', 'mysecret').update(`${ts}.${body}`).digest('hex');
    assert.equal(opts.headers['X-Stellar_Card-Signature'], `sha256=${expected}`);
  });

  it('omits signature headers when no webhookSecret', async () => {
    await fireWebhook('https://hooks.example.com/events', { status: 'ok' }, null);
    const { opts } = fetchCalls[0];
    assert.ok(!opts.headers['X-Stellar_Card-Signature']);
    assert.ok(!opts.headers['X-Stellar_Card-Timestamp']);
  });
...
// $0.50 on a failed order. This is a financial correctness bug: stellar_card
