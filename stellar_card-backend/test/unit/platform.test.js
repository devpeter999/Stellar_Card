...
describe('isPlatformOwner', () => {
  ...
  it('returns false for a non-matching email', () => {
    process.env.CARDS402_PLATFORM_OWNER_EMAIL = 'ops@stellar_card.test';
    assert.equal(isPlatformOwner('user@example.com'), false);
  });

  it('returns true for an exact email match', () => {
    process.env.CARDS402_PLATFORM_OWNER_EMAIL = 'ops@stellar_card.test';
    assert.equal(isPlatformOwner('ops@stellar_card.test'), true);
  });
  ...
  it('F1: returns false on an object input', () => {
    process.env.CARDS402_PLATFORM_OWNER_EMAIL = 'ops@stellar_card.test';
    assert.equal(isPlatformOwner(/** @type {any} */ ({ email: 'ops@stellar_card.test' })), false);
  });

  it('F1: returns false on an array input', () => {
    process.env.CARDS402_PLATFORM_OWNER_EMAIL = 'ops@stellar_card.test';
    assert.equal(isPlatformOwner(/** @type {any} */ (['ops@stellar_card.test'])), false);
  });
  ...
  it('F1: returns false when CARDS402_PLATFORM_OWNER_EMAIL is set to a non-string', () => {
    ...
    assert.equal(isPlatformOwner('ops@stellar_card.test'), false);
  });
